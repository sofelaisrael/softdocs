...
export function getAdjacentDocs(sections: NavSection[], docsDir: string, currentSlug: string): { prev: NavItem | null; next: NavItem | null } {
  const flat: NavItem[] = []
  for (const section of sections) {
    if (section.slug && fs.existsSync(path.join(docsDir, section.slug, 'index.mdx'))) {
      flat.push({
        title: section.label,
        slug: section.slug,
      })
    } else if (!section.slug && fs.existsSync(path.join(docsDir, 'index.mdx'))) {
